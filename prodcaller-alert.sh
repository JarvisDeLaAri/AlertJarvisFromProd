#!/bin/bash
# prodcaller-alert.sh — SSH forced command that posts alerts to JarvisHub
# Runs on: HUB SERVER (where JarvisHub is running)
#
# This script is triggered automatically when prodcaller SSHes in.
# It parses the SSH command and POSTs to the local JarvisHub API.
#
# SSH authorized_keys entry:
#   command="/usr/local/bin/prodcaller-alert.sh",no-port-forwarding,no-X11-forwarding,no-agent-forwarding,no-pty <PUBLIC_KEY>
